...
TYPE_REGEX=re.compile('^type (?P<type_name>[^\ ]*) (?P<type_target>.*)$')
...
class StructParser(object):
    ...
    def __init__(self, version, file_paths=[], start_keys=[], product='nomad', override_field={}, tr_types={}):
    ...
    def download_file(self, file):
        return requests.get(self.DOWNLOAD_PREFIX + self.product + '/' + self.version + file)
    ...
    def parse_file(self, file):
        for line in self.download_file(file).text.splitlines():
            ...
            if self.current_struct:
                ...
                if field_def:
                    value =  self.tr_types.get(field_def.group('field_type').strip(), field_def.group('field_type').strip())
